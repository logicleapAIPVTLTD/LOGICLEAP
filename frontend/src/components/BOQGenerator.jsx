...
import { Upload, FileText, Image, X, Trash2, CheckCircle, ChevronDown, ChevronRight, Building2, Layers } from "lucide-react";
...
const BOQGenerator = ({ setActiveView, setSelectedBOQItems }) => {
  ...
  const removeWorkGroup = (workName) => {
    const updated = generatedBOQ.filter(item => {
      ...
      return itemWorkName.toLowerCase().trim() !== workName.toLowerCase().trim();
    });
    setGeneratedBOQ(updated);
  ...
  const handleGenerateFromImages = async () => {
    if (!formData.projectName || !formData.location || !formData.state || !formData.projectType) {
    ...
    if (isNaN(meterPerPixel) || meterPerPixel <= 0) {
      setError("Invalid meterPerPixel value. Must be a positive number (e.g., 0.01 for 1cm per pixel)");
      return;
    }
    ...
    try {
      ...
      const response = await boqAPI.generateFromImages(floorPlanFile, layoutFile, projectInfo);
      ...
      if (response.success && response.data) {
        ...
        // Backend returns: { success: true, data: { boq: [...], rooms: [...], available_works: {...}, ... } }
        // Extract BOQ items from response.data.boq
        const boqArray = response.data.boq || response.data.items || [];
        
        console.log("BOQ Array extracted:", boqArray);
        console.log("Rooms data:", response.data.rooms);
        console.log("Available works:", response.data.available_works);
        console.log("Room summary:", response.data.room_summary);
        ...
        const boqItems = Array.isArray(boqArray) && boqArray.length > 0
          ? boqArray.map((item, index) => ({
              itemNo: index + 1,
              work: item.work_name || item.work || item.description || item.item_name || item.scope_label || `Item ${index + 1}`,
              space: item.space || "",
              workCode: item.work_code || "",
              quantity: item.qty_sqm || item.qty_sqft || item.quantity || 0,
              unit: item.unit || "sqm",
              qtySqm: item.qty_sqm || 0,
              qtySqft: item.qty_sqft || 0,
              rate: item.rate || "",
              amount: item.amount || (item.rate && item.quantity ? (parseFloat(item.rate) * parseFloat(item.quantity)).toFixed(2) : ""),
        ...
        if (boqItems.length > 0 || (response.data.rooms && response.data.rooms.length > 0)) {
          setShowResults(true);
        } else {
          setError("No BOQ items or rooms detected. Please check your images and try again.");
        }
      } else {
        setError(response.error || response.message || "Failed to generate BOQ from images");
      }
    } catch (err) {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const boqItems = boqArray.map((item, index) => ({
          itemNo: index + 1,
          work: item.work_name || item.work || item.description || item.item_name || item.material || `Item ${index + 1}`,
          space: item.space || "",
          workCode: item.work_code || "",
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
        ...
        <p className="text-gray-600">Generate Bill of Quantities from images or upload existing BOQ</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-800">Generate from Images</h3>
                  <p className="text-sm text-gray-600">Upload Floor Plan + 2D Layout</p>
                </div>
              </div>
              <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-1 rounded">Recommended</span>
            </div>
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-800">Upload Existing BOQ</h3>
      ...
      {inputMode === "images" && !showResults && (
        <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold text-gray-800">Generate from Images</h3>
            <button
              onClick={resetForm}
          ...
          <div className="space-y-4 mb-6">
            <h4 className="font-semibold text-gray-700 mb-3">Project Information</h4>
          ...
          <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Scale Factor (Meter per Pixel) <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                name="meterPerPixel"
              ...
              <p className="text-xs text-gray-500 mt-1">
                Enter the scale factor: meters represented by each pixel (e.g., 0.01 = 1cm per pixel, 0.001 = 1mm per pixel)
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-2">Determines how much work is needed (dimensions, areas)</p>
              <div
              ...
              >
                <input
                  ...
                  accept="image/*"
                  className="hidden"
                  onChange={(e) => handleFileChange("floorPlan", e.target.files[0])}
                />
                {floorPlanFile ? (
                  <div className="space-y-2">
                    <CheckCircle className="w-12 h-12 text-green-600 mx-auto" />
                    <p className="text-sm font-medium text-green-700">{floorPlanFile.name}</p>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setFloorPlanFile(null);
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-green-600 font-medium">Drop Floor Plan here or click to upload</p>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                2D Layout / Design Drawing <span className="text-red-500">*</span>
              </label>
              <p className="text-xs text-gray-500 mb-2">Determines what work needs to be done (specifications, materials)</p>
              <div
                onDragOver={handleDragOver}
              ...
              <input
                  ...
                  accept="image/*"
                  className="hidden"
                  onChange={(e) => handleFileChange("layout", e.target.files[0])}
                />
                {layoutFile ? (
                  <div className="space-y-2">
                    <CheckCircle className="w-12 h-12 text-green-600 mx-auto" />
                    <p className="text-sm font-medium text-green-700">{layoutFile.name}</p>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setLayoutFile(null);
                  ...
                  <div className="space-y-2">
                    <Upload className="w-12 h-12 text-green-500 mx-auto" />
                    <p className="text-sm text-green-600 font-medium">Drop 2D Layout here or click to upload</p>
                    <p className="text-xs text-gray-500">PNG, JPG, JPEG</p>
                  </div>
                )}
              </div>
          ...
          >
            {loading ? "Generating BOQ from Images..." : "Generate BOQ from Images"}
          </button>
        </div>
      )}
      ...
      {inputMode === "upload" && !showResults && (
        <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold text-gray-800">Upload Existing BOQ</h3>
            <button
              onClick={resetForm}
              className="text-gray-400 hover:text-gray-600"
            >
          ...
          <div className="mb-6">
            ...
            <p className="text-xs text-gray-500 mb-2">Upload a pre-existing BOQ document from your sales team</p>
            <div
              onDragOver={handleDragOver}
            ...
            >
              ...
              {boqFile ? (
                <div className="space-y-2">
                  ...
                  <p className="text-sm font-medium text-green-700">{boqFile.name}</p>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setBoqFile(null);
                ...
                <div className="space-y-2">
                  <Upload className="w-12 h-12 text-green-500 mx-auto" />
                  <p className="text-sm text-green-600 font-medium">Drop BOQ file here or click to upload</p>
                  <p className="text-xs text-gray-500">Excel (.xlsx, .xls), CSV, or PDF</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-xl font-semibold text-gray-800">Generated BOQ Results</h3>
                <p className="text-sm text-gray-600 mt-1">
                  {backendData.total_rooms || 0} rooms detected • {generatedBOQ.length} BOQ items generated
                </p>
              </div>
                      <button 
                onClick={() => {
            ...
            {backendData.room_summary && (
              <div className="mb-4 p-3 bg-green-50 rounded-lg border border-green-200">
                <p className="text-sm font-medium text-gray-700 mb-2">Room Summary:</p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(backendData.room_summary).map(([space, count]) => (
                    <span key={space} className="px-3 py-1 bg-white text-green-700 rounded-full text-xs font-medium border border-green-200">
                      {space}: {count}
                    </span>
                  ))}
                </div>
          ...
          {/* Rooms Section */}
          {backendData.rooms && Array.isArray(backendData.rooms) && backendData.rooms.length > 0 && (
            <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
                      <button 
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-semibold text-gray-800">Detected Rooms ({backendData.rooms.length})</h3>
                </div>
                {expandedSections.rooms ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                      </button>
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-green-50 border-b-2 border-green-200">
                        <th className="p-3 text-left font-semibold text-gray-700">Room Name</th>
                        <th className="p-3 text-right font-semibold text-gray-700">Area (sqm)</th>
                        <th className="p-3 text-right font-semibold text-gray-700">Area (sqft)</th>
                        <th className="p-3 text-center font-semibold text-gray-700">Confidence</th>
                        <th className="p-3 text-left font-semibold text-gray-700">Area Source</th>
                        <th className="p-3 text-center font-semibold text-gray-700">Has Dimensions</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white">
                      {backendData.rooms.map((room, index) => (
                        <tr key={index} className="border-b border-green-100 hover:bg-green-50/30 transition-colors">
                          <td className="p-3 text-gray-800 font-medium capitalize">{room.name || "Room"}</td>
                          <td className="p-3 text-right text-gray-700">{room.sqm ? parseFloat(room.sqm).toFixed(2) : "-"}</td>
                          <td className="p-3 text-right text-gray-700">{room.sqft ? parseFloat(room.sqft).toFixed(2) : "-"}</td>
                          <td className="p-3 text-center text-gray-700">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              room.confidence >= 0.7 ? 'bg-green-100 text-green-700' :
                              room.confidence >= 0.4 ? 'bg-yellow-100 text-yellow-700' :
                              'bg-red-100 text-red-700'
                            }`}>
                              {(room.confidence * 100).toFixed(0)}%
                            </span>
                          </td>
                          <td className="p-3 text-gray-600 text-xs capitalize">{room.area_source || "-"}</td>
          ...
          {backendData.available_works && Object.keys(backendData.available_works).length > 0 && (
            <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
              <button
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-semibold text-gray-800">Available Works</h3>
                </div>
                {expandedSections.availableWorks ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
              </button>

              {expandedSections.availableWorks && (
                <div className="space-y-4">
                  {Object.entries(backendData.available_works).map(([spaceType, works]) => (
                    <div key={spaceType} className="border border-green-200 rounded-lg p-4">
                      <h4 className="font-semibold text-gray-800 mb-2 capitalize">{spaceType} ({works.length} works)</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                        {Array.isArray(works) && works.map((work, idx) => (
                          <div key={idx} className="p-2 bg-green-50 rounded text-xs text-gray-700 border border-green-100">
                            {work.work_name || work.work_code || `Work ${idx + 1}`}
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {generatedBOQ.length > 0 && (
            <div className="bg-white border border-green-200 rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-semibold text-gray-800">BOQ Items ({generatedBOQ.length} items)</h3>
                  <p className="text-sm text-gray-600 mt-1">Grouped by work type - Click to expand/collapse</p>
                </div>
              </div>

              <div className="space-y-3">
                {Object.entries(groupedBOQ).map(([workName, items]) => (
                  <div key={workName} className="border border-green-200 rounded-lg overflow-hidden">
                    ...
                    <div className="w-full flex items-center justify-between p-4 bg-green-50 hover:bg-green-100 transition-colors">
                      ...
                      >
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-xs text-gray-600 mt-0.5">
                            ...
                            {items[0]?.workCode && ` Code: ${items[0].workCode}`}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (window.confirm(`Are you sure you want to delete all "${workName}" items?`)) {
                          ...
                          <table className="w-full text-sm border-collapse">
                            <thead>
                              <tr className="bg-gray-50 border-b border-green-200">
                                <th className="p-2 text-left font-semibold text-gray-700 text-xs">Item No.</th>
                                <th className="p-2 text-left font-semibold text-gray-700 text-xs">Space</th>
                                <th className="p-2 text-left font-semibold text-gray-700 text-xs">Work Code</th>
                                <th className="p-2 text-right font-semibold text-gray-700 text-xs">Qty (sqm)</th>
                                <th className="p-2 text-right font-semibold text-gray-700 text-xs">Qty (sqft)</th>
                                <th className="p-2 text-left font-semibold text-gray-700 text-xs">Area Source</th>
                                <th className="p-2 text-center font-semibold text-gray-700 text-xs">Confidence</th>
                                <th className="p-2 text-center font-semibold text-gray-700 text-xs">Action</th>
                              </tr>
                            </thead>
                            <tbody>
                              {items.map((item, idx) => (
                                <tr key={idx} className="border-b border-green-100 hover:bg-green-50/30 transition-colors">
                                  <td className="p-2 text-gray-700">{item.itemNo || idx + 1}</td>
                                  <td className="p-2 text-gray-700 capitalize">{item.space || "-"}</td>
                                  <td className="p-2 text-gray-600 text-xs">{item.workCode || "-"}</td>
                                  <td className="p-2 text-right text-gray-700">{item.qtySqm ? parseFloat(item.qtySqm).toFixed(2) : "-"}</td>
                                  <td className="p-2 text-right text-gray-700">{item.qtySqft ? parseFloat(item.qtySqft).toFixed(2) : "-"}</td>
                                  <td className="p-2 text-gray-600 text-xs capitalize">{item.areaSource || "-"}</td>
                                  ...
                                  <td className="p-2 text-center">
                                    <button
                                      onClick={() => removeBOQItem(item.originalIndex)}
              ...
              {generatedBOQ.length === 0 && (
                <div className="mb-6 p-4 text-center text-gray-500 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-sm">No BOQ items remaining. Please go back and generate again.</p>
                </div>
              )}
